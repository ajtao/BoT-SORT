...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cls, jumping, feat=None, frame_id=None):
        ...
        self.features = {}
        if feat is not None:
            self.update_features(feat, frame_id=frame_id)
        self.alpha = 0.9
    ...
    def update_features(self, feat, frame_id):
        ...
        assert frame_id not in self.features
        self.features[frame_id] = feat
...
class ConfSORT(object):
    def __init__(self,
                 args,
                 frame_rate=30,
                 mark_confusion=False
                 ):
        """ Customize for splitting confused tracks """
        global class_names
        ...
        self.confused_tracks = []
        self.in_confusion = False

    def update(self, output_results, img, fnum):
        """
        output_results = (N, {bbox, score, cls, feats})
        """
        self.frame_id += 1
        assert fnum == self.frame_id
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        removed_stracks = []
        ...
        if len(dets) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, j, f, frame_id=self.frame_id) for
                              (tlbr, s, c, j, f) in zip(dets, scores_keep, classes_keep,
                                                        jumping_keep, features_keep)]
            else:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, j, frame_id=self.frame_id) for
        ...
        if True:
            """
            Track confusion happens when a single detection matches more than one track.
            """
            thresh = 1.0 - self.args.conf_thresh
            thresh_ious = ious_dists < thresh
            track_ids = np.array([s.track_id for s in strack_pool])
            any_confusion = False
            # Whenever we have more than one track satisfy the IOU confusion threshold,
            # we consider it a questionable match.
            for det_idx in range(len(detections)):
                ti = thresh_ious[:, det_idx]
                if ti.sum() > 1:
                    any_confusion = True

                    if not self.in_confusion:
                        self.in_confusion = 1
                        self.confused_tracks = []
                        self.fnum_start = self.frame_id

                    track_indices = np.where(ti > 0)
                    confusion_track_ids = track_ids[track_indices]
                    self.confused_tracks.extend(confusion_track_ids.tolist())
                    confusion_ious = (ious_dists[track_indices, det_idx]*100).astype(int)
                    print(f'fnum {self.frame_id} multi-track match for det {det_idx}: '
                          f'matches {ti.astype(int)}, track ids: {confusion_track_ids} '
                          f'ious: {confusion_ious}')

            if not any_confusion:
                if self.in_confusion:
                    fnum_start = self.fnum_start
                    fnum_end = self.frame_id
                    split_event = fnum_start, fnum_end, set(self.confused_tracks)
                self.in_confusion = False
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, j, frame_id=self.frame_id) for
        ...
        return output_stracks, split_event
